...
#SBATCH --array=1-1 # Specify the range of array jobs (e.g., 0-2 for 3 configs)

# Command(s) to run:

# Define an array of config files
CONFIG_FILES=(
    '../configs/baselines/train-single-layer_seq-cpg-sparsified.gin'
    '../configs/baselines/train-single-layer_seq-cpg.gin'
    '../configs/baselines/train-single-layer_seq-only.gin'
)
