...
def test_train_predict_integration(config_file):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Only rank 0 has the training run's actual temp_dir/checkpoints (ModelCheckpoint only
        # saves from global_rank 0); Lightning's subprocess DDP launcher re-invokes this entire
        # pytest test per additional rank, and each such re-invocation creates its own unrelated
        # tempfile.TemporaryDirectory(), so non-zero ranks have nothing meaningful to assert here.
